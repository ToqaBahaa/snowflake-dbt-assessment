...
- **Models (`models/`)**:
  ...
  - **customer_revenue** (Gold layer)  
    - This model combines TPCH_SF1 `orders`, `customer`  and `lineitem` to aggregate total revenue per customer.  
    - Tests:
...
2. This projects reads data from SNOWFLAKE_SAMPLE_DATA.TPCH_SF1 so make sure it is available in your SnowFlake account
  ```sql
  SELECT COUNT(*) 
  FROM SNOWFLAKE_SAMPLE_DATA.TPCH_SF1.LINEITEM;
  ```
3. Log in / Sign up for a **dbt Cloud account** and connect it to your Snowflake account
4. Create a new project on dbt Cloud and configure its connection to use **your** warehouse, database and schema
5. Connect the project to this GitHub repository (snowflake-dbt-assessment) to sync the models, sources and schema files
